...
class PConfig {
 ...
 public:
  ...
  std::uint32_t max_clients = 10000;     // 10000
  std::uint32_t slow_log_time = 1000;    // 1000 microseconds
  std::uint32_t slow_log_max_len = 128;  // 128
  std::uint32_t master_port;             // replication
  std::string include_file;                    // the template config
  std::vector<PString> modules;                 // modules
  std::int32_t fast_cmd_threads_num = 4;
  std::int32_t slow_cmd_threads_num = 4;
  std::uint64_t max_client_response_size = 1073741824;
  std::uint64_t small_compaction_threshold = 604800;
  std::uint64_t small_compaction_duration_threshold = 259200;

  bool daemonize = false;
  std::string pid_file = "./pikiwidb.pid";
  std::string ip = "127.0.0.1";
  std::uint16_t port = 9221;
  std::uint16_t raft_port_offset = 10;
